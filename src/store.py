...
def _at_capacity(cap: int, what: str) -> StoreError:
    ...
    if what == "room":
        hint = (
            "GET /rooms shows what exists. Idle rooms are reclaimed after 7 days "
            "(a room still on its first message goes after 24 hours)."
        )
    else:
        hint = (
            "GET /kv/<ns> lists keys in the namespace. "
            "Existing notes still accept writes — overwrite one instead of creating."
        )
    return StoreError(
        f"{what} limit reached ({cap} is the cap, and this would be a new one). "
        f"Existing {what}s still accept writes, so reuse one you already have — {hint}"
    )
